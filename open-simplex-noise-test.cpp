...
static int write_png_image(const char *filename, unsigned char *pixels, int w, int h, int has_alpha)
{
	...
	row = static_cast<png_byte**>(png_malloc(png_ptr, h * sizeof(*row)));
	for (y = 0; y < h; y++) {
		row[y] = static_cast<png_byte*>(png_malloc(png_ptr, w * bytes_per_pixel));
		for (x = 0; x < w; x++) {
...
int main(__attribute__((unused)) int argc, __attribute__((unused)) char *argv[])
{
	...
	for (y = 0; y < HEIGHT; y++) {
		for (x = 0; x < WIDTH; x++) {
			...
			value = open_simplex_noise4(ctx, (float) x / FEATURE_SIZE, (float) y / FEATURE_SIZE, 0.0, 0.0);
#else
			/* Use three octaves: frequency N, N/2 and N/4 with relative amplitudes 4:2:1. */
			v0 = open_simplex_noise4(&ctx, (float) x / FEATURE_SIZE / 4,
						(float) y / FEATURE_SIZE / 4, 0.0, 0.0);
			v1 = open_simplex_noise4(&ctx, (float) x / FEATURE_SIZE / 2,
						(float) y / FEATURE_SIZE / 2, 0.0, 0.0);
			v2 = open_simplex_noise4(&ctx, (float) x / FEATURE_SIZE / 1,
			...
			value = open_simplex_noise2(&ctx, (float) x / FEATURE_SIZE, (float) y / FEATURE_SIZE);
			rgb = 0x010101 * (uint32_t) ((value + 1) * 127.5);
			image3d[y][x] = (0x0ff << 24) | (rgb);

			value = open_simplex_noise3(&ctx, (float) x / FEATURE_SIZE, (float) y / FEATURE_SIZE, 0.0);
			rgb = 0x010101 * (uint32_t) ((value + 1) * 127.5);
